...
The only issue regarding throughput right now, and also mass automation, is this system's requirement of manually loading tabs to solve this. The system is not headless. Perhaps a solution like dockering with a virtual framebuffer could do this, or some sort of standard equivalent. However, this would usally not make a major difference as the Cloudflare challenges result in mainly a CPU bottleneck, and CPU performance would only recieve a minor boost from this. Plus, this would also take a lot of work to do, and eliminate OS level gui clicking. You'd have to click at the browser level. 
...
Might make a script to manage and lock the z-index for open tabs. This way, tabs always stay locked in the same level placement compared to others, which could allow you to overlap tabs almost entirely, since the cf turnstile checkbox loads only in the top corner. The concern with overlapping tabs right now is that interacting with a browser (like our os level click) makes the z-index rise to the top and makes it render above others, which can mess up orientation. Locking z-index would fix this.
